...
# def setup_logger_kwargs(exp_name, seed=None, data_dir=None, datestamp=False,
#                         env_name=None, algo=None, exp_num=None):
...
#     if env_name is not None and algo is not None and exp_num is not None:
#         env_part = valid_str(env_name)
#         algo_part = valid_str(algo)
#         exp_part = valid_str(exp_num)
#         if seed is not None:
#             run_part = f"{exp_part}-s{seed}"
#         else:
#             run_part = exp_part
#         relpath = osp.join(env_part, algo_part, run_part)
#         if datestamp:
#             hms_time = time.strftime("%Y-%m-%d_%H-%M-%S")
#             subfolder = ''.join([hms_time, '-', exp_name, '_s', str(seed)])
#             relpath = osp.join(relpath, subfolder)
